...
public class ItemService {
    ...
    public void saveItem(ItemDTO itemDTO) throws DuplicateIdentifierException, FailedOperationException {
        try {
            ...
            PreparedStatement stmt = connection.prepareStatement("INSERT INTO item (code,description,unit_price,qty_on_hand,unit_type,image_path) VALUES (?,?,?,?,?,?)");
            ...
            stmt.setString(6, itemDTO.getFilePath());
            stmt.executeUpdate();
    ...
    public void updateItem(ItemDTO itemDTO) throws FailedOperationException, NotFountException {
        ...
        try {
            ...
            PreparedStatement pstm = connection.prepareStatement("UPDATE item SET description=?, unit_price=?, qty_on_hand=?, unit_type=?, image_path=? WHERE code=?");
            pstm.setString(1,itemDTO.getDescription());
            pstm.setBigDecimal(2,itemDTO.getUnitPrice());
            pstm.setInt(3,itemDTO.getQtyOnHand());
            pstm.setString(4,itemDTO.getUnitType());
            pstm.setString(5,itemDTO.getFilePath());
    ...
    public ItemDTO findItem(String code) throws NotFountException, FailedOperationException {
        try {
            ...
            rst.next();
            return new ItemDTO(code,rst.getString("description"),rst.getBigDecimal("unit_price"),rst.getInt("qty_on_hand"),rst.getString("unit_type"), rst.getString("image_path"));
        } catch (SQLException e) {
    ...
    public List<ItemDTO> findAllItems() throws FailedOperationException {
        try {
            ...
            while (rst.next()){
                itemList.add(new ItemDTO(rst.getString("code"),rst.getString("description"),rst.getBigDecimal("unit_price"),rst.getInt("qty_on_hand"),rst.getString("unit_type"), rst.getString("image_path")));
            }
